...
static gwServices_PollControl_PacketPending_t
	*gwServices_GetPacketQueueEntry (uint64_t ieeeAddr);
...
static gwServices_PollControl_PacketPending_t
	*gwServices_AddPacketQueueEntry (void);
void gwServices_ClearPacketQueueEntry (uint64_t ieeeAddr);
uint8 gwServices_StartStateMachine_DeviceAttrList (int connection,
												 SrvrDeviceInfoT *
												 pDeviceInfo,
												 GwGetDeviceAttributeListReq *
												 pGetAttrListReq);
uint8
gwServices_UpdateStateMachine_DeviceAttrList (GwGetDeviceAttributeListRspInd *
											pGetAttrListRsp,
											uint16 appTransId,
											uint16 inClusterId,
											zclDiscoverAttrsRspCmd_t *
											pDiscAttrsRsp);
static uint8 gwServices_AddAttrList (gwServices_StateMachine_AttrList_t *
									 pState, uint16 clusterId,
									 zclDiscoverAttrsRspCmd_t * pDiscAttrRsp);
static gwServices_StateMachine_AttrList_t *gwServices_FindStateMachine (uint64_t
																		ieeeAddr);
...
static bool gwServices_FindClustersOnEndpoint (SrvrDeviceInfoT * pDeviceInfo,
											 gwServices_StateMachine_AttrList_t
											 * pState);
static void gwServices_FinishStateMachine (gwServices_StateMachine_AttrList_t *
										 pState);
static void gwServices_FreeStateMachine (gwServices_StateMachine_AttrList_t *
										 pState);
static void gwServices_FreeAttrList (sGwServices_AttrList_t * pAttrListRecord);
static GwClusterListT *gwServices_ConvertDiscAttrsRspPb (uint16 clusterId,
														 zclDiscoverAttrsRspCmd_t
														 * pDiscAttrRsp,
														 uint16 startAttr,
														 GwClusterListT *
...
GwClusterListT **gwServices_ConvertPbClusterList (int attrCount,
												zclAttrRec_t * pAttrRecord,
												uint8 inClusterCount,
												uint32 * pInClusterList,
												uint32 * pClusterListCount);
void gwServices_FreePbClusterList (int clusterListCount,
...
static void gwServices_FreeAttrListTable (gwServices_StateMachine_AttrList_t *
										pTableEntry);
...
bool gwServices_SetPacketQueueEntry (uint64_t ieeeAddr)
{
	...
	if (gGwService_PacketPendingQueue.pHead)
	{
		...
		gGwService_PacketPendingQueue.pQueueList =
			gGwService_PacketPendingQueue.pHead;

		while (gGwService_PacketPendingQueue.pQueueList)
        {
            if (gGwService_PacketPendingQueue.pQueueList->ieeeAddr ==
                ieeeAddr)
            {
                // set packet pending bit
                gGwService_PacketPendingQueue.pQueueList->packetPending =
                    TRUE;
            ...
            gGwService_PacketPendingQueue.pQueueList =
                gGwService_PacketPendingQueue.pQueueList->pNext;
        }
	}
												 ...
												 pGetAttrListReq)
{
	...
	uiPrintfEx (trDEBUG,
				"Starting Attribute List State Machine - IeeeAddr: %016llX",
	...
	if (gwServices_FindClustersOnEndpoint (pDeviceInfo, pAttrListState))
	{
		...
		status = sendGwGetDeviceAttributeListReq (connection, pGetAttrListReq,
													pAttrListState->
													pServerClusterList[0],
													pAttrListState->startAttr);
											...
											pDiscAttrsRsp)
{
	...
	// Check to see if state machine is already in use for this IEEE addresss
	pState =
		gwServices_FindStateMachine (pGetAttrListRsp->srcaddress->ieeeaddr);
	if (!pState)
	{
		uiPrintf
	...
	if (pState->clusterIndex < pState->clusterCount)
	{
		...
		GwGetDeviceAttributeListReq getAttrListReq =
			GW_GET_DEVICE_ATTRIBUTE_LIST_REQ__INIT;
		...
		if (ZSuccess ==
			sendGwGetDeviceAttributeListReq (pState->connection,
											 &getAttrListReq, nextClusterId,
											 pState->startAttr))
        {
																		...
																		ieeeAddr)
{
	...
	{
		if ((gpGwServices_StateMachine_AttrList[i].state !=
			 gwServices_State_Available_c)
...
static gwServices_StateMachine_AttrList_t *gwServices_GetStateMachine (void)
{
	...
	{
		uiPrintfEx (trDEBUG,
					"Attribute list: Error - Can't allocate more state machines\n");
									 ...
									 zclDiscoverAttrsRspCmd_t * pDiscAttrRsp)
{
	...
	pClusterList =
		gwServices_ConvertDiscAttrsRspPb (clusterId, pDiscAttrRsp,
										pState->startAttr,
										pNewAttrListRecord->pClusterList);
	...
	else
	{
		...
		pState->startAttr =
			pDiscAttrRsp->attrList[pDiscAttrRsp->numAttr - 1].attrID + 1;
	}

	uiPrintfEx (trDEBUG,
				"Update State Machine: Cluster Index: %d, Remaining Clusters: %d\n",
										 ...
										 pState)
{
	...
	GwGetDeviceAttributeListRspInd getAttrListRsp =
		GW_GET_DEVICE_ATTRIBUTE_LIST_RSP_IND__INIT;
	...
	else
	{
		...
		sendGwGetDeviceAttributeListRspInd (pState->connection,
											&getAttrListRsp);
	}
	...
	for (i = 0; i < pState->clusterCount; i++)
	{
		...
		while (pState->pAttrList)
        {
            ...
            if (pState->pServerClusterList[i] ==
                pState->pAttrList->pClusterList->clusterid)
            {
														 ...
														 pClusterList)
{
	...
	else
	{
		...
		pAttrList =
			realloc (pNewClusterList->attributelist,
					 (pNewClusterList->n_attributelist +
						pDiscAttrRsp->numAttr) * sizeof (uint32));
	...
	{
		pAttrList[pNewClusterList->n_attributelist + i] =
			pDiscAttrRsp->attrList[i].attrID;
	}
