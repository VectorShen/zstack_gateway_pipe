...
static ZDSecMgrEntry_t fixedEntry = { 0 };
static aps_Group_t dummyGroup = { 0 };
							 ...
							 cId_t * OutClusterList, byte SecurityEnable)
{
	...
	if (pBuf)
	{
		...
		*pPtr++ = NumOutClusters;
		...
		status =
			sendNPIExpectDefaultStatusZDO (MT_ZDO_MATCH_DESC_REQ, len, pBuf);
								...
								byte TcSignificance, byte SecurityEnable)
{
	...
	status =
		sendNPIExpectDefaultStatusZDO (MT_ZDO_MGMT_PERMIT_JOIN_REQ, 5, buf);
						 ...
						 byte len, uint8 * buf, byte SecurityEnable)
{
	...
	if (pBuf)
	{
		...
		*pPtr++ = len;
		...
		status =
			sendNPIExpectDefaultStatusZDO (MT_ZDO_SEND_DATA, bufLen, pBuf);
								 ...
								 uint8 NwkUpdateId, uint16 NwkManagerAddr)
{
	...
	status =
		sendNPIExpectDefaultStatusZDO (MT_ZDO_MGMT_NWK_UPDATE_REQ, 11, buf);
...
ZDO_NwkIEEEAddrResp_t *ZDO_ParseAddrRsp (zdoIncomingMsg_t * inMsg)
{
	...
	// Make buffer
	rsp =
		(ZDO_NwkIEEEAddrResp_t *) osal_mem_alloc (sizeof (ZDO_NwkIEEEAddrResp_t)
												+ (cnt * sizeof (uint16)));
...
ZDO_ActiveEndpointRsp_t *ZDO_ParseEPListRsp (zdoIncomingMsg_t * inMsg)
{
	...
	pRsp =
		...
		osal_mem_alloc (sizeof (ZDO_ActiveEndpointRsp_t) + cnt);
	if (pRsp)
	{
		pRsp->status = Status;
...
ZDO_MgmtLqiRsp_t *ZDO_ParseMgmtLqiRsp (zdoIncomingMsg_t * inMsg)
{
	...
	pRsp = (ZDO_MgmtLqiRsp_t *) osal_mem_alloc (sizeof (ZDO_MgmtLqiRsp_t)
												+
												(neighborLqiCount *
												 sizeof (ZDP_MgmtLqiItem_t)));
	if (pRsp)
	{
		uint8 x;
...
ZDO_MgmtRtgRsp_t *ZDO_ParseMgmtRtgRsp (zdoIncomingMsg_t * inMsg)
{
	...
	// Allocate a buffer big enough to handle the list
	pRsp =
		(ZDO_MgmtRtgRsp_t *) osal_mem_alloc (sizeof (ZDO_MgmtRtgRsp_t) +
											 (rtgListCount *
											sizeof (rtgItem_t)));
	if (pRsp)
	{
...
ZDO_MgmtBindRsp_t *ZDO_ParseMgmtBindRsp (zdoIncomingMsg_t * inMsg)
{
	...
	// Allocate a buffer big enough to handle the list
	pRsp =
		(ZDO_MgmtBindRsp_t *) osal_mem_alloc ((sizeof (ZDO_MgmtBindRsp_t)
											 +
											 (bindingListCount *
												sizeof (apsBindingItem_t))));
	if (pRsp)
	{
		uint8 x;
		...
		for (x = 0; x < bindingListCount; x++)
        {
            ...
            {
                pList->dstAddr.addr.shortAddr =
                    BUILD_UINT16 (msg[0], msg[1]);
                msg += 2;
            }
...
ZDO_UserDescRsp_t *ZDO_ParseUserDescRsp (zdoIncomingMsg_t * inMsg)
{
	...
	pRsp =
		(ZDO_UserDescRsp_t *) osal_mem_alloc (sizeof (ZDO_UserDescRsp_t) +
											descLen);
	if (pRsp)
	{
		pRsp->status = msg[0];
...
uint8 ZDO_ParseSimpleDescBuf (uint8 * buf, SimpleDescriptionFormat_t * desc)
{
	...
	{
		if (!
			(desc->pAppInClusterList =
			 (uint16 *) osal_mem_alloc (num * sizeof (uint16))))
        {
 ...
 *          by the calling function [osal_mem_free()].
 */
ZDO_MgmtNwkUpdateNotify_t *ZDO_ParseMgmtNwkUpdateNotify (zdoIncomingMsg_t *
														 inMsg)
{
	uint8 status;
	...
	pRsp =
		...
		osal_mem_alloc (sizeof (ZDO_MgmtNwkUpdateNotify_t) + listCount);
...
ZStatus_t ZDSecMgrEntryLookupExt (uint8 * extAddr, ZDSecMgrEntry_t ** entry)
{
	...
	*entry = NULL;
	...
	pRsp = apicSendSynchData (ZNP_API_CLIENT, MT_RPC_SYS_ZDO,
							MT_ZDO_SEC_ENTRY_LOOKUP_EXT, 8, extAddr, NULL,
							&rspcmdid, NULL);
...
uint8 aps_FindAllGroupsForEndpoint (uint8 endpoint, uint16 * groupList)
{
	...
	// send serialized request to NPI synchronously
	pRsp = apicSendSynchData (ZNP_API_CLIENT, MT_RPC_SYS_ZDO,
							MT_ZDO_EXT_FIND_ALL_GROUPS_ENDPOINT, 1, &endpoint,
							NULL, &rspcmdid, NULL);
...
aps_Group_t *aps_FindGroup (uint8 endpoint, uint16 groupID)
{
	...
	// send serialized request to NPI synchronously
	pRsp = apicSendSynchData (ZNP_API_CLIENT, MT_RPC_SYS_ZDO,
							MT_ZDO_EXT_FIND_GROUP, 3, pData, NULL, &rspcmdid,
							NULL);
	...
	if (pRsp)
	{
		if (rspcmdid == MT_ZDO_EXT_FIND_GROUP)
        {
            ...
            if (status == ZSuccess)
            {
                ...
                {
                    memcpy (&dummyGroup.name[1], &pRsp[4],
                            dummyGroup.name[0]);
                }
...
ZStatus_t ZDSecMgrAPSRemove (uint16 nwkAddr, uint8 * extAddr, uint16 parentAddr)
{
	...
	status =
		(uint8) sendNPIExpectDefaultStatusZDO (MT_ZDO_EXT_SEC_APS_REMOVE_REQ,
											 12, buf);
...
void zdoExtNwkInfo (nwkIB_t * pNib)
{
	...
	pBuf =
		apicSendSynchData (ZNP_API_CLIENT, MT_RPC_SYS_ZDO, MT_ZDO_EXT_NWK_INFO,
