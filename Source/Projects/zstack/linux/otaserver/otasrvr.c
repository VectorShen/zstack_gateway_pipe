						...
						uint8 * pData, uint8 type)
{
	...
	{
		if (appConnectionHandle == -1)
			appConnectionHandle = connection;
		return;
	}
		...
		case OTA_MGR_CMD_ID_T__OTA_UPDATE_IMAGE_REGISTERATION_REQ:
			{
				...
				pUpdateReq =
					ota_update_image_registeration_req__unpack (NULL, len,
																pData);
				...
				if (pUpdateReq)
                {
                    ...
                    ota_update_image_registeration_req__free_unpacked
                        (pUpdateReq, NULL);
                }
			}
			break;
		...
		case OTA_MGR_CMD_ID_T__OTA_UPDATE_ENABLE_REQ:
			{
				...
				pUpdateEnable =
					ota_update_enable_req__unpack (NULL, len, pData);
				if (pUpdateEnable)
                {
                    processUpdateEnable (connection, pUpdateEnable);

                    ota_update_enable_req__free_unpacked (pUpdateEnable,
                                                        NULL);
                }
			}
			break;
		...
		case OTA_MGR_CMD_ID_T__OTA_UPDATE_APPLY_IMAGE_REQ:
			{
				...
				pApplyImage =
					ota_update_apply_image_req__unpack (NULL, len, pData);

				if (pApplyImage)
                {
                    processApplyImage (connection, pApplyImage);

                    ota_update_apply_image_req__free_unpacked (pApplyImage,
                                                                NULL);
                }
			}
			break;
								...
								OtaUpdateImageRegisterationReq * pMsg)
{
	...
	if (pMsg->registerunregister)
	{
		...
		if (pMsg->has_executiondelay)
			executiondelay = pMsg->executiondelay;

		if (pMsg->has_executiontime)
			executiontime = pMsg->executiontime;

		/* Call OtaServer_AddImage */
		osStatus =
			OtaServer_AddImage ((char *)(pMsg->imagepath),
								pMsg->executetiming, executiondelay,
								executiontime,
								pMsg->updatesupporteddevicelist,
								pMsg->n_supporteddevicelist,
								pMsg->supporteddevicelist, pMsg->notification,
								connection);
	...
	if (pBuf)
	{
		...
		APIS_SendData (connection, ZSTACK_OTASYS_IDS__RPC_SYS_PB_OTA_MGR,
						 FALSE, OTA_MGR_CMD_ID_T__ZIGBEE_GENERIC_CNF, len,
						 pBuf);
...
static void processUpdateEnable (int connection, OtaUpdateEnableReq * pMsg)
{
	...
	if (pBuf)
	{
		...
		APIS_SendData (connection, ZSTACK_OTASYS_IDS__RPC_SYS_PB_OTA_MGR,
						 FALSE, OTA_MGR_CMD_ID_T__OTA_UPDATE_ENABLE_CNF, len,
						 pBuf);
...
static void processApplyImage (int connection, OtaUpdateApplyImageReq * pMsg)
{
	...
	else if (pMsg->address->addrmode == ADDRESS_MODE__UNICAST)
	{
		...
		if (pMsg->address->has_ieeeaddr)
        {
            ...
            memcpy (pSrcAddr->addr.extAddr, &pMsg->address->ieeeaddr,
                    Z_EXTADDR_LEN);
        }
		else
        {
...
void processApplyImageCnf (int connection, bool status)
{
	...
	if (status == TRUE)
	{
		...
		imgCnf.status = GENERIC_STATUS__FAILURE;
