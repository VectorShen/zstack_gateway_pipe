...
void OTA_GetFileName (char *pName, zclOTA_FileID_t * pFileId, char *text)
{
	...
	else
	{
        ...
        {
            *pName++ =
                HEX_char[UINT16_BREAK_HEX (pFileId->manufacturer, i)];
        }
	}
...
uint8 *OTA_StreamToFileId (zclOTA_FileID_t * pFileId, uint8 * pStream)
{
	if (pStream)
	{
		...
		pStream += 2;
		pFileId->version =
			BUILD_UINT32 (pStream[0], pStream[1], pStream[2], pStream[3]);
		pStream += 4;
	}
