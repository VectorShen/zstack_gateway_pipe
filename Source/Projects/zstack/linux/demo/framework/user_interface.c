...
void ui_process_device_list (int *selected_device_index,
...
void console_event_handler (void *arg)
{
	...
	for (i = 0; i < num_of_chars; i++)
	{
            ...
            if ((c == 65) || (c == 66) || (c == 67) || (c == 68))
            {
                ...
                switch (c)
                {
                    case 65:
                        ...
                        {
                            ui_process_device_list
                                (&selected_device_index,
                                    &prev_valid_device_index,
                                    &next_valid_device_index);

                            if (prev_valid_device_index !=
                                MAX_DEVICE_LIST)
                            {
                                ds_device_table
                                    [selected_device_index].
                            ...
                            {
                                ui_print_status
                                    (UI_STATUS_NOTIFICATION_TIMEOUT,
                                        "Reached top of device list");
                            }
                        }
                        else
                        {
                            ui_print_status
                                (UI_STATUS_NOTIFICATION_TIMEOUT,
                                    "Use [`] to switch to Device Selection");
                        }
                        break;
                    case 66:
                        if (selection_mode == SELECTION_MODE_DEVICES)
		...
		if (escape_state == 1)
        {
            ...
            {
                escape_state = 2;
                tu_set_timer (&escape_sequence_timeout,
                                ESCAPE_SEQUENCE_TIMEOUT, false,
                                escape_sequence_timeout_handler, NULL);
            }
            else
            {
                escape_state = 0;
		...
		if (escape_state == 0)
        {
            switch (c)
            {
                case ' ':
                    current_action =
                        (current_action + 1) % NUM_OF_ACTIONS;
                    ui_redraw_actions ();
                    break;
                case 'q':
                case 'Q':
                ...
                case 'W':
                    ...
                    if (selected_device_index != MAX_DEVICE_LIST)
                    {
                        ...
                        destination_addr.ieee_addr =
                            ds_device_table[selected_device_index].
                            ieee_addr;
                        destination_addr.endpoint =
                            ds_device_table[selected_device_index].
                            ep_list[ds_device_table
                                    [selected_device_index].
                                    selected_endpoint_index].
                            endpoint_id;
                ...
                case 'A':
                    ...
                    UI_PRINT_LOG ("sensor_reading_acive=%d",
                                    sensor_reading_acive);
                    LOCATE (SECTION_ACTIONS, 12);
                    printf (DARK_WHITE "%s",
                            sensor_reading_acive ? "A" : "=");

                    if (sensors_found && sensor_reading_acive)
                    {
                        if (!sensor_read_periodic_timer.in_use)
                        {
                            tu_set_timer (&sensor_read_periodic_timer,
                                            SENSOR_READ_INTERVAL, true,
                        ...
                        if (selected_device_index != MAX_DEVICE_LIST)
                        {
                            ...
                            destination_addr.endpoint =
                                ...
                                ep_list[ds_device_table
                                        ...
                                        selected_endpoint_index].
                                endpoint_id;
                            attr_configure_reporting (destination_addr,
                                                    gateway_self_addr,
                                                    5,
                            ...
                            attr_configure_reporting (destination_addr,
                                                    gateway_self_addr,
                                                    5,
                                                    CLUSTER_ID_HUMIDITY_MEASUREMENT,
                                                    ATTRID_HUMDITY,
                ...
                case '+':
                    action_value[current_action] =
                        (action_value[current_action] +
                            1) % ACTION_VALUE_MAX[current_action];
                    ui_redraw_actions ();
                ...
                case ']':
                    ...
                    {
                        ui_print_status (UI_STATUS_NOTIFICATION_TIMEOUT,
                                            "Please record a macro first");
                    }
                    else
                    {
                    ...
                    break;
                case '=':
                    printf (SCROLLING_ALL CURSOR_SAVE "\x1B[999B"
                            "\x1B[999C" CURSOR_GET_POS CURSOR_RESTORE);
                    break;
                case '-':
                    action_value[current_action] =
                        (action_value[current_action] +
                            ACTION_VALUE_MAX[current_action] -
                            1) % ACTION_VALUE_MAX[current_action];
                    ui_redraw_actions ();
                    if ((fast_action_enabled)
                        && ((current_action == ACTION_LVL)
                ...
                case '*':
                    ...
                    UI_PRINT_LOG ("fast_action_enabled=%d",
                                    fast_action_enabled);
                    LOCATE (SECTION_ACTIONS, 10);
                    printf (DARK_WHITE "%s",
                            fast_action_enabled ? "*" : "=");
                    break;
                case '?':
                    UI_PRINT_HELP_START (DARK_BLUE
                                            "Usage help start -------------------------------------------------");
                    UI_PRINT_HELP (DARK_BLUE
                                        "- Either Devices, Groups or Scenes are selectable at a given time.");
                    UI_PRINT_HELP (DARK_BLUE "  " LIGHT_BLUE "[`]"
                                        DARK_BLUE
                                        " - toggle between the three selection modes (Devices/Groups/Scenes).");
                    UI_PRINT_HELP (DARK_BLUE
                                        "  The section title of the current selection mode is highlighted with a brighter white, and marked with [].");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[UP/DOWN]"
                                        DARK_BLUE " - select a device,");
                    UI_PRINT_HELP (DARK_BLUE "  " LIGHT_BLUE
                                        "[LEFT/RIGHT]" DARK_BLUE
                                        " - select an endpoint/group/scene");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[SPACE]"
                                        DARK_BLUE
                                        " toggles between the available actions,");
                    UI_PRINT_HELP (DARK_BLUE "  " LIGHT_BLUE "[+/-]"
                                        DARK_BLUE
                                        " changes the value associated with the action,");
                    UI_PRINT_HELP (DARK_BLUE "  " LIGHT_BLUE "[ENTER]"
                                        DARK_BLUE
                    ...
                    UI_PRINT_HELP (DARK_BLUE
                                        "  First it will select the binding source device, Second it will select the binding destination and perform the action.");
                    UI_PRINT_HELP (DARK_BLUE
                                        "  if the selected binding destination is the same as the source, the operation is canceled.");
                    UI_PRINT_HELP (DARK_BLUE
                                        "- Actions that requires destination address will use unicast, groupcast or broadcast addressing,");
                    UI_PRINT_HELP (DARK_BLUE
                                        "  depending on the current addressing mode (binding and device removal always use unicast)");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[A/a]"
                                        DARK_BLUE
                                        " - Automatically read data from all sensors periodically.");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[x]"
                                        DARK_BLUE " - Perform a soft reset.");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[X]"
                                        DARK_BLUE " - Perform a hard reset.");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[K/k]"
                                        DARK_BLUE
                                        " - Shutdown the gateway subsystem.");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[O/o]"
                                        DARK_BLUE
                                        " - Initiate OTA operation.");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[B/b]"
                                        DARK_BLUE
                                        " - Toggle between addressing modes (Unicast/Groupcast/Broadcast");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[*]"
                                        DARK_BLUE
                                        " - Enable/Disable immediate execution. (When enabled, '*' appears on the actions title line.)");
                    UI_PRINT_HELP (DARK_BLUE
                                        "  When immediate execution is enabled, actions will be triggered as soon as [+/-] is pressed, after the respective value is updated.");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[[]"
                                        DARK_BLUE
                    ...
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[1]"
                                        DARK_BLUE " - Display less log lines");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[2]"
                                        DARK_BLUE " - Display more log lines");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[3]"
                                        DARK_BLUE
                                        " - Display less device lines, but more log lines");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[4]"
                                        DARK_BLUE
                                        " - Display more device lines, but less log lines");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[=]"
                                        DARK_BLUE
                                        " - Redraw user interface according to actual terminal dimentions");
                    UI_PRINT_HELP (DARK_BLUE "- " LIGHT_BLUE "[z]"
                                        DARK_BLUE
            ...
            if (perform_action)
            {
                    ...
                    || (current_action == ACTION_SCENE_RECALL))
                {
                    ...
                    if (addressing_ok)
                    {
                        ...
                        {
                            case ACTION_ON_OFF:
                                act_set_onoff (&destination_addr,
                                                action_value
                                                [current_action]);
                                break;
                            case ACTION_LVL:
                                act_set_level (&destination_addr, 0,
                                                action_value
                                                [current_action]);
                                break;
                            case ACTION_HUE:
                            case ACTION_SAT:
                                act_set_color (&destination_addr,
                                                action_value
                                                [ACTION_HUE],
                                                action_value
                                                [ACTION_SAT]);
                            ...
                            case ACTION_SCENE_RECALL:
                                gs_recall_scene (&destination_addr,
                                                    selected_group,
                                                    selected_scene);
                                break;
                            default:
                            ...
                            || (current_action == ACTION_REMOVE))
                {
                    ...
                    if (selected_device_index != MAX_DEVICE_LIST)
                    {
                        ...
                        {
                            case ACTION_REMOVE:
                                comm_remove_device_request
                                    (&destination_addr);
                                break;
                            case ACTION_BIND_UNBIND:
                                ui_process_bind (action_value
                                                    [current_action],
                                                    selected_device_index);
                                break;
                            default:
...
void ui_print_status (uint64_t timeout_ms, char *format, ...)
{
	...
	if (timeout_ms > 0)
	{
		tu_set_timer (&status_line_timeout, UI_STATUS_NOTIFICATION_TIMEOUT,
						false, ui_clear_status_line, NULL);
	}
	else
	{
...
bool ui_is_device_a_smart_plug (device_info_t * device, uint8_t * sensor_ep)
{
	...
	if (ui_is_valid_device_record (device))
	{
        ...
        {
            if (device->ep_list[ep_index].profile_id ==
                PROFILE_ID_HOME_AUTOMATION)
            {
                if ((device->ep_list[ep_index].device_id ==
                        DEVICE_ID_MAINS_POWER_OUTLET)
                    || (device->ep_list[ep_index].device_id ==
                        DEVICE_ID_NON_STANDARD_MAINS_POWER_OUTLET))
                {
                    for (cluster_index = 0;
                            cluster_index <
                            device->ep_list[ep_index].num_ip_clusters;
                            cluster_index++)
                    {
                        if (device->ep_list[ep_index].
                            ip_cluster_list[cluster_index].
                            cluster_id ==
                            CLUSTER_ID_SMART_ENERGY_METERING)
                        {
                            if (sensor_ep != NULL)
                            {
                                *sensor_ep =
                                    device->ep_list[ep_index].
                                    endpoint_id;
                            }
                            return true;
                        }
...
bool ui_is_device_a_temperature_sensor (device_info_t * device,
										uint8_t * sensor_ep)
{
	int ep_index;
	int cluster_index;
	...
	if (ui_is_valid_device_record (device))
	{
		for (ep_index = 0; ep_index < device->num_endpoints; ep_index++)
        {
            ...
            {
                if (device->ep_list[ep_index].device_id ==
                    DEVICE_ID_HVAC_TEMPERATURE_SENSOR)
                {
                    for (cluster_index = 0;
                            cluster_index <
                            device->ep_list[ep_index].num_ip_clusters;
...
bool ui_is_device_a_humidity_sensor (device_info_t * device,
									 uint8_t * sensor_ep)
{
	int ep_index;
	int cluster_index;
...
bool ui_is_device_an_occupancy_sensor (device_info_t * device,
									 uint8_t * sensor_ep)
{
	int ep_index;
	int cluster_index;
...
void ui_redraw_network_info (void)
{
	...
	{
		printf (DARK_MAGENTA "EXTPANID:" LIGHT_MAGENTA
				"%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X",
				(unsigned int)((ds_network_status.ext_pan_id >> 0) & 0xFF),
				(unsigned int)((ds_network_status.ext_pan_id >> 8) & 0xFF),
				(unsigned int)((ds_network_status.ext_pan_id >> 16) & 0xFF),
				(unsigned int)((ds_network_status.ext_pan_id >> 24) & 0xFF),
				(unsigned int)((ds_network_status.ext_pan_id >> 32) & 0xFF),
				(unsigned int)((ds_network_status.ext_pan_id >> 40) & 0xFF),
				(unsigned int)((ds_network_status.ext_pan_id >> 48) & 0xFF),
				(unsigned int)((ds_network_status.ext_pan_id >> 56) & 0xFF));
		printf (DARK_MAGENTA " PANID:" LIGHT_MAGENTA "0x%04X",
				ds_network_status.pan_id);
		printf (DARK_MAGENTA " CHANNEL:" LIGHT_MAGENTA "%d",
				ds_network_status.nwk_channel);
	...
	{
		printf (LIGHT_MAGENTA "%-*s", UI_NETWORK_LINE_LENGTH,
...
void ui_send_sensor_read (device_info_t * device)
{
	...
	if ((device == last_sensor_read)
		&& ui_is_device_a_humidity_sensor (device, &sensor_ep))
	{
		destination_addr.ieee_addr = device->ieee_addr;
		destination_addr.endpoint = sensor_ep;
		attr_id[0] = ATTRID_HUMDITY;
		attr_send_read_attribute_request (&destination_addr,
											CLUSTER_ID_HUMIDITY_MEASUREMENT, 1,
											attr_id);
	}
	...
	else if (ui_is_device_an_occupancy_sensor (device, &sensor_ep))
	{
		...
		attr_send_read_attribute_request (&destination_addr,
											CLUSTER_ID_OCCUPANCY_SENSING, 1,
											attr_id);
	}
	else if (ui_is_device_a_smart_plug (device, &sensor_ep))
	{
		...
		attr_send_read_attribute_request (&destination_addr,
											CLUSTER_ID_SMART_ENERGY_METERING, 5,
											attr_id);
	}
...
{
	memset (list_of_sensors_pending_read, MAX_DEVICE_LIST,
...
void ui_sensor_read_state_machine (bool timed_out, void *arg)
{
	...
	else
	{
		...
		while ((current_read_sensor < MAX_SUPPORTED_SENSORS)
				 && (list_of_sensors_pending_read[current_read_sensor] !=
					 MAX_DEVICE_LIST)
				 &&
				 (!ui_is_device_a_sensor
				(&ds_device_table
				 [list_of_sensors_pending_read[current_read_sensor]])))
        {
            UI_PRINT_LOG
                ("Device list has changed, index %d does not point to a sensor now",
                    list_of_sensors_pending_read[current_read_sensor]);
            current_read_sensor++;
        }
		...
		if ((current_read_sensor < MAX_SUPPORTED_SENSORS)
			&& (list_of_sensors_pending_read[current_read_sensor] !=
				MAX_DEVICE_LIST))
        {
            ui_send_sensor_read (&ds_device_table
                                    [list_of_sensors_pending_read
                                [current_read_sensor]]);
            UI_PRINT_LOG ("sensor read - read request sent to 0x%04X",
                        ds_device_table[list_of_sensors_pending_read
                                        [current_read_sensor]].nwk_addr);
        }
		else
        {
            si_unregister_idle_callback ();
	...
	else
	{
		...
		for (i = 0; i < MAX_DEVICE_LIST; i++)
        {
            if (ui_is_valid_device_record (&ds_device_table[i])
                && ui_is_device_a_sensor (&ds_device_table[i]))
            {
                if (ui_is_device_a_humidity_sensor
                    (&ds_device_table[i], NULL)
                    &&
                    ui_is_device_a_temperature_sensor (&ds_device_table
                                                            [i], NULL))
                {
...
void ui_redraw_selection_mode (void)
{
	switch (selection_mode)
	{
		case SELECTION_MODE_DEVICES:
			LOCATE (SECTION_SCENES_TOP, 1);
			ui_draw_separation_line (DARK_WHITE);
			printf ("=SCENES");
			LOCATE (SECTION_DEVICES_TOP_0, 1);
			ui_draw_separation_line (DARK_WHITE);
			printf ("=" LIGHT_WHITE "[DEVICES]");
			break;
		case SELECTION_MODE_GROUPS:
			LOCATE (SECTION_DEVICES_TOP_0, 1);
			ui_draw_separation_line (DARK_WHITE);
			printf ("=DEVICES");
			LOCATE (SECTION_GROUPS_TOP, 1);
			ui_draw_separation_line (DARK_WHITE);
			printf ("=" LIGHT_WHITE "[GROUPS]");
			break;
...
void ui_redraw_device_list (void)
{
	...
	if (selected_device_number > 0)
	{
		...
		while ((devicelist_offset + section_devices_lines) <
	...
	for (i = 0;
		 (i < MAX_DEVICE_LIST)
		 && ((devices_found - devicelist_offset) < section_devices_lines); i++)
	{
		if (ui_is_valid_device_record (&ds_device_table[i]))
        {
            devices_found++;
            ...
            if (devices_found > devicelist_offset)
            {
                ...
                STRING_ADD_64BIT_HEX (device_string,
                                        ds_device_table[i].ieee_addr);
                STRING_ADD (device_string, " %04X ",
                            ds_device_table[i].nwk_addr);

                for (ep_index = 0;
                        ep_index < ds_device_table[i].num_endpoints;
                        ep_index++)
                {
                    if (ep_index > 0)
                    {
                    ...
                    if ((ep_index ==
                            ds_device_table[i].selected_endpoint_index)
                        && (ds_device_table[i].selected
                            || ds_device_table[i].
                            selected_as_bind_destination))
                    {
                    ...
                    if (ds_device_table[i].ep_list[ep_index].
                        profile_id == PROFILE_ID_HOME_AUTOMATION)
                    {
                        switch (ds_device_table[i].ep_list[ep_index].
                                device_id)
                        {
                            case DEVICE_ID_NON_STANDARD_MAINS_POWER_OUTLET:
                            case DEVICE_ID_MAINS_POWER_OUTLET:
                                STRING_ADD (device_string, "%02X",
                                            ds_device_table[i].
                                            ep_list[ep_index].
                                            endpoint_id);
                                STRING_ADD (device_string,
                    ...
                    else if (ds_device_table[i].ep_list[ep_index].
                                profile_id == PROFILE_ID_ZIGBEE_LIGHT_LINK)
                    {
                        switch (ds_device_table[i].ep_list[ep_index].
                                device_id)
                        {
                    ...
                    else
                    {
                        STRING_ADD (device_string, "%02X",
                                    ds_device_table[i].
                                    ep_list[ep_index].endpoint_id);
                        STRING_ADD (device_string, " %04X(%04X)",
                                    ds_device_table[i].
                                    ep_list[ep_index].device_id,
                                    ds_device_table[i].
                                    ep_list[ep_index].profile_id);
                    }
                    ...
                    if ((ep_index ==
                        ...
                        && (ds_device_table[i].selected
                            ...
                            selected_as_bind_destination))
                    {
                        STRING_ADD (device_string, "%s",
                                    NO_UNDERSCORE);
                    }

                    for (j = 0;
                            j <
                            ds_device_table[i].ep_list[ep_index].
                            num_ip_clusters; j++)
                    {
                        for (k = 0; k < MAX_ATTRIBUTES; k++)	//ds_device_table[i].ep_list[ep_index].ip_cluster_list[j].num_attributes; k++)
                        {
                            if (ds_device_table[i].
                                ep_list[ep_index].
                                ip_cluster_list[j].
                                attribute_list[k].valid)
                            {
                                switch (ds_device_table[i].
                                        ep_list[ep_index].
                                        ip_cluster_list[j].
                                        cluster_id)
                                {
                                    case ZCL_CLUSTER_ID_MS_TEMPERATURE_MEASUREMENT:
                                        switch (ds_device_table
                                                [i].
                                                ep_list
                                                [ep_index].
                                                ip_cluster_list
                                    ...
                                    case ZCL_CLUSTER_ID_SE_SIMPLE_METERING:
                                                ...
                                                [k].attr_id)
                                        {
                                            ...
                                            case ATTRID_SE_METERING_FORMATTING_SUMMATION_FORMATING:
                                                ...
                                                leading_zeros
                                                    =
                                                    ((summation_formatting & 0x80) == 0);
                                                right_digits =
                                                    (summation_formatting
                                                        & 0x07);
                                                left_digits =
                                                    ((summation_formatting >> 3) & 0x0F);
                                                break;

                                            case ATTRID_SE_METERING_INSTANTANEOUS_DEMAND:
                                                power =
                                                    LITTLE_ENDIAN_TO_INT24
                                                    (ds_device_table
                                                        [i].
                                                        ep_list
                                                        [ep_index].
                ...
                LOCATE (SECTION_DEVICES_TOP + devices_found -
                        devicelist_offset, 1);
                //printf("%s%-*s", (ds_device_table[i].selected ? LIGHT_RED ">" : DARK_RED " "), ui_device_line_length, device_string);

                printf (RESET_ATTRIBUTES "%s%s%-*s",
                        ds_device_table[i].
                        selected_as_bind_destination ? GREEN : RED,
                        (ds_device_table[i].selected ? BOLD ">" : " "),
                        ui_device_line_length, device_string);
            }
        }
	}
	...
	if (sensors_found && sensor_reading_acive)
	{
		if (!sensor_read_periodic_timer.in_use)
        {
            tu_set_timer (&sensor_read_periodic_timer, SENSOR_READ_INTERVAL,
                        true, sensor_read_handler, NULL);
            sensor_read_handler (NULL);
        }
	}
	else
							 ...
							 int *next_valid_device_index)
{
	...
	if ((selected_device_index == NULL) | (prev_valid_device_index == NULL)
		|| (next_valid_device_index == NULL))
	{
		return;
	}
...
void ui_redraw_addressing_mode (void)
{
	LOCATE (SECTION_ACTIONS, 17);
	printf (DARK_WHITE "%s",
			(addressing_mode ==
			 ADDRESSING_MODE_BROADCAST) ? "(BROADCAST)" : (addressing_mode ==
														 ADDRESSING_MODE_GROUPCAST)
			? "(GROUPCAST)" : "(UNICAST)==");
}
	...
	for (i = 0; i < NUM_OF_ACTIONS; i++)
	{
		...
		switch (ACTION_TYPE[i])
        {
            case ACTION_TYPE_INTEGER:
                printf ("%s[%.*s" UNDERSCORE "%c" NO_UNDERSCORE "%s:%03d] ",
                        (i == current_action ? LIGHT_YELLOW : DARK_YELLOW),
                        strchr (ACTION_NAMES[i], '&') - ACTION_NAMES[i],
                        ACTION_NAMES[i], strchr (ACTION_NAMES[i], '&')[1],
                        strchr (ACTION_NAMES[i], '&') + 2, action_value[i]);
                break;
            ...
            case ACTION_TYPE_STRING:
                ...
                for (j = 0; j < ACTION_VALUE_MAX[i]; j++)
                {
                    printf ("%s%.*s" UNDERSCORE "%c" NO_UNDERSCORE "%s",
                            ...
                            : ((i ==
                                current_action) ? LIGHT_YELLOW :
                                    DARK_YELLOW),
                            strchr (ACTION_SUBNAMES[i][j],
                                    '&') - ACTION_SUBNAMES[i][j],
                            ACTION_SUBNAMES[i][j],
...
{
	LOCATE (SECTION_ACTIONS, 10);
	printf (DARK_WHITE "%s", fast_action_enabled ? "*" : "=");
	LOCATE (SECTION_ACTIONS, 12);
	printf (DARK_WHITE "%s", sensor_reading_acive ? "A" : "=");
	LOCATE (SECTION_ACTIONS, 13);
	printf (DARK_WHITE "%s", record_macro ? "R" : "=");
	LOCATE (SECTION_ACTIONS, 14);
	printf (DARK_WHITE "%s", assign_macro ? "M" : "=");
}
...
void ui_draw_screen_layout (void)
{
	...
	LOCATE (SECTION_TITLE_TOP, 1);
	ui_draw_title_line (LIGHT_WHITE);
	printf (LIGHT_WHITE "*** TI GATEWAY DEMO APP v%d.%02d ",
			HA_GW_VERSION_MAJOR, HA_GW_VERSION_MINOR);
	LOCATE (SECTION_NETWORK_TOP, 1);
	ui_draw_separation_line (DARK_WHITE);
	printf ("=NETWORK");
	LOCATE (SECTION_DEVICES_TOP_0, 1);
	ui_draw_separation_line (DARK_WHITE);
	printf ("=DEVICES");
	LOCATE (SECTION_DEVICES_TOP, 1);
	ui_draw_subtitle_line (DARK_WHITE);
	printf (NO_UNDERSCORE " " UNDERSCORE "IEEE_ADDR              " NO_UNDERSCORE
			" " UNDERSCORE "ADDR" NO_UNDERSCORE " " UNDERSCORE
	...
	ui_draw_separation_line (DARK_WHITE);
	printf ("=SCENES");
	LOCATE (SECTION_ACTIONS, 1);
	ui_draw_separation_line (DARK_WHITE);
	printf ("=ACTIONS");
	LOCATE (SECTION_LOG_TOP, 1);
	ui_draw_separation_line (DARK_WHITE);
	printf ("=LOG");
	LOCATE (SECTION_STATUS, 1);
	ui_draw_separation_line (DARK_WHITE);
	printf ("=APPLICATION-STATUS");
	LOCATE (SECTION_HELP_TOP, 1);
	ui_draw_separation_line (DARK_WHITE);
	printf ("=HELP");

	LOCATE (SECTION_HELP_TOP + 1, 1);
	printf (DARK_BLUE "Press " LIGHT_BLUE "[?]" DARK_BLUE
			" to display usage help. ");
	LOCATE (SECTION_HELP_TOP + 2, 1);
	printf (DARK_BLUE
			"If the log window is small, press [?] again to see more help.");
...
int ui_init (char *log_filename)
{
	...
	{
		printf (CURSOR_HOME "\x1B[%dB" "\x1B[%dC" CURSOR_GET_POS CURSOR_HOME,
				999, 999);
		...
		else
        {
            ...
            if ((pos_row < UI_CURRENT_MAX_USED_LINE)
                || (pos_col < UI_MIN_LINE_LENGTH))
            {
                printf
                    ("Terminal dimentions are not optimal - it is currently set to %d rows by %d columns.\n"
                        ...
                        pos_row, pos_col, UI_CURRENT_MAX_USED_LINE,
                        UI_MIN_LINE_LENGTH);
	...
	UI_PRINT_LOG (BOLD
				"TI Home Automation Gateway Sample Application, version %d.%d",
				HA_GW_VERSION_MAJOR, HA_GW_VERSION_MINOR);
...
void ui_process_bind (binding_mode_t binding_mode, int selected_device_index)
{
	...
	{
		if (ui_is_valid_device_record (&ds_device_table[i])
			&& ds_device_table[i].selected_as_bind_destination)
        {
	...
	if (binding_source_device_index == MAX_DEVICE_LIST)
	{
		...
		ds_device_table[selected_device_index].selected_as_bind_destination =
			true;
		ui_redraw_device_list ();
	}
	...
	{
		UI_PRINT_LOG ("Sending %s command",
						binding_mode == BINDING_MODE_BIND ? "bind" : "unbind");

		source_addr.ieee_addr =
			ds_device_table[binding_source_device_index].ieee_addr;
		source_addr.endpoint =
			ds_device_table[binding_source_device_index].
			ep_list[ds_device_table[binding_source_device_index].
					selected_endpoint_index].endpoint_id;

		dst_addr.ieee_addr = ds_device_table[selected_device_index].ieee_addr;
		dst_addr.endpoint =
			ds_device_table[selected_device_index].
			ep_list[ds_device_table[selected_device_index].
					selected_endpoint_index].endpoint_id;

		comm_device_binding_entry_request (&source_addr, &dst_addr,
											 CLUSTER_ID_ONOFF, binding_mode);

		ds_device_table[binding_source_device_index].
			selected_as_bind_destination = false;
