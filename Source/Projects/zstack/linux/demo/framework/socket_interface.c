...
int si_register_idle_callback (si_idle_calback_t idle_cb, void *idle_cb_arg)
{
	...
	*current_idle_cb_entry_ptr = malloc (sizeof (idle_cb_element));
	...
	if (current_idle_cb_entry != NULL)
	{
		...
		UI_PRINT_LOG
			("Created new idle_cb entry (#%d) for func (%08x), arg (%08X)",
			 idle_cb_count, (int)IDLE_CB, (int)IDLE_CB_ARG);
	}
	else
	{
		UI_PRINT_LOG
			("ERROR: Failed to create new idle_cb entry (#%d) for func (%08x)",
			 idle_cb_count, (int)IDLE_CB);
		return -1;
	}
												 ...
												 confirmation_timeout_interval)
{
	...
	server_details->confirmation_timeout_interval =
		confirmation_timeout_interval;
}

int si_send_packet (pkt_buf_t * pkt,
					confirmation_processing_cb_t _confirmation_processing_cb,
					void *_confirmation_processing_arg)
{
	...
	{
		UI_PRINT_LOG
			("ERROR unknown subsystem ID %d. Following packet discarded:",
			 pkt->header.subsystem);
	...
	{
		ui_print_status (0,
						 "BUSY - please wait for previous operation to complete");
		return -1;
	}

	if (tcp_send_packet
		(server, (uint8_t *) pkt,
		 pkt->header.len + sizeof (pkt_buf_hdr_t)) == -1)
	{
	...
	tu_set_timer (&confirmation_wait_timer,
				server->confirmation_timeout_interval, false,
				confirmation_timeout_handler, NULL);
...
void ui_print_packet_to_log (pkt_buf_t * pkt, char *prefix, char *hilight)
{
	...
	while (remaining_len--)
	{
		...
		if (STRING_REMAINING_CHARS (log_string) <
			(2 + ((remaining_len > 0) ? 1 : 0)))
        {
            UI_PRINT_LOG ("%s%s", hilight, log_string);
            STRING_START (log_string, "     ");
        }
#else
		if ((STRING_REMAINING_CHARS (log_string) <= 4)
			&& (remaining_len >= 1))
        {
            STRING_ADD (log_string, "...");
            break;
        }
		else if ((STRING_REMAINING_CHARS (log_string) <= 7)
				 && (remaining_len >= 2))
        {
...
void si_nwk_manager_incoming_data_handler (pkt_buf_t * pkt, int len)
{
	switch (pkt->header.cmd_id)
	{
		...
		default:
			UI_PRINT_LOG
				("Unsupported incoming command id from nwk manager server (cmd_id %d)",
				 (pkt->header.cmd_id));
			break;
	}
}
...
void si_gateway_incoming_data_handler (pkt_buf_t * pkt, int len)
{
	switch (pkt->header.cmd_id)
	{
		...
		default:
			UI_PRINT_LOG
				("Unsupported incoming command id from gateway server (cmd_id %d)",
				 (pkt->header.cmd_id));
			break;
	}
}
...
void si_ota_incoming_data_handler (pkt_buf_t * pkt, int len)
{
	switch (pkt->header.cmd_id)
	{
		...
		default:
			UI_PRINT_LOG
				("Unsupported incoming command id from ota server (cmd_id %d)",
				 (pkt->header.cmd_id));
			break;
	}
}
...
void nwk_mgr_server_connected_disconnected_handler (void)
{
	...
	network_manager_server.confirmation_timeout_interval =
	...
	{
		tu_set_timer (&init_state_machine_timer,
						INIT_STATE_MACHINE_STARTUP_DELAY, false,
						init_state_machine_startup_handler, NULL);
	}
}
...
{
	if (tcp_new_server_connection
		(&network_manager_server, nwk_manager_server_hostname,
		 (server_incoming_data_handler_t) si_nwk_manager_incoming_data_handler,
		 "NWK_MGR", nwk_mgr_server_connected_disconnected_handler) == -1)
	{
	...
	if (tcp_new_server_connection
		(&gateway_server, gateway_server_hostname,
		 (server_incoming_data_handler_t) si_gateway_incoming_data_handler,
		 "GATEWAY", gateway_server_connected_disconnected_handler) == -1)
	{
...
#if 1
	if (tcp_new_server_connection
		(&ota_server, ota_server_hostname,
		 (server_incoming_data_handler_t) si_ota_incoming_data_handler, "OTA",
		 ota_server_connected_disconnected_handler) == -1)
	{
...
void si_delay_next_idle_state_machine_state (uint64_t milliseconds)
{
	...
	tu_set_timer (&attribute_reporting_req_wait_timer, milliseconds, false,
				si_idle_callbacl_timer_handler, NULL);
}
