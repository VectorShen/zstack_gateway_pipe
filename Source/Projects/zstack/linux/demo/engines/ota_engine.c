...
void ota_enable_state_machine (bool timed_out, void *arg)
{
	...
	if ((arg == NULL) && (state != 0))
	{
		ui_print_status (UI_STATUS_NOTIFICATION_TIMEOUT,
						 "OTA-Enable already in progress");
		return;
	}
	...
	{
		UI_PRINT_LOG
			("ota_enable_state_machine: ERROR: OTA server not connected");
		state = 6;
	}
	...
	while (rerun_state_machine)
	{
        ...
        {
            case 1:
                if (si_register_idle_callback
                    (ota_enable_state_machine, NOT_NULL) == 0)
                {
                ...
                else
                {
                    state = 0;
                    UI_PRINT_LOG
                        ("WARNING: OTA-Enable state machine cannot be initialized at this time");
                }
                break;
            case 2:
                enable_ota_upgrade ();
...
static void enable_ota_upgrade (void)
{
	...
	if (pkt)
	{
		...
		if (si_send_packet
			(pkt, (confirmation_processing_cb_t) & update_enable_cnf,
			 NULL) != 0)
        {
...
static void update_enable_cnf (pkt_buf_t * pkt, void *cbarg)
{
	...
	{
		UI_PRINT_LOG
			("update_enable_cnf: Received bad header: Not OTA_MGR_CMD_ID_T__OTA_UPDATE_ENABLE_CNF");
		return;
	}

	UI_PRINT_LOG
		("update_enable_cnf: Received OTA_MGR_CMD_ID_T__OTA_UPDATE_ENABLE_CNF");
...
void upgrade_apply_image (zb_addr_t * addr)
{
	...
	if (pkt)
	{
		...
		if (si_send_packet (pkt,
							...
							upgrade_apply_image_cnf, NULL) != 0)
        {
...
void ota_process_download_finished_indication (pkt_buf_t * pkt)
{
	...
	msg =
		ota_update_dl_finished_ind__unpack (NULL, pkt->header.len,
											pkt->packed_protobuf_packet);
	...
	if (msg)
	{
        ...
        {
            UI_PRINT_LOG
                ("ota_process_download_finished_indication: Status SUCCESS.");

            UI_PRINT_LOG ("addr=%s",
                        ui_make_string_OtaAddrStruct (msg->address));
        ...
        {
            UI_PRINT_LOG
                ("ota_process_download_finished_indication: FAILURE (%d)",
                    msg->status);
        }
	...
	{
		UI_PRINT_LOG
			("ota_process_download_finished_indication: Error Could not unpack msg");
	}
}
...
static void upgrade_apply_image_cnf (pkt_buf_t * pkt, void *cbarg)
{
	...
	UI_PRINT_LOG
		("upgrade_apply_image_cnf: Received OTA_MGR_CMD_ID_T__ZIGBEE_GENERIC_CNF");

	msg =
		ota_zigbee_generic_cnf__unpack (NULL, pkt->header.len,
										pkt->packed_protobuf_packet);
...
static void upgrade_process_regis_cnf (pkt_buf_t * pkt, void *cbarg)
{
	...
	UI_PRINT_LOG
		("upgrade_process_regis_cnf: Received IMAGE REGIS OTA_MGR_CMD_ID_T__ZIGBEE_GENERIC_CNF for index %d",
		 index);
	...
	if (msg)
	{
		...
		if (msg->status == GENERIC_STATUS__SUCCESS)
        {
            UI_PRINT_LOG ("OTA Server successfully registered file #%d",
                        index);
        ...
        {
            //ERROR 
            UI_PRINT_LOG ("ERROR: OTA Server could not register file #%d",
                        index);
            upgrade_file_status[index].fileStatus = -1;
        }
	...
	{
		UI_PRINT_LOG
			("upgrade_process_regis_cnf: Error: Could not unpack msg");
	}
}
...
static void parse_file_to_list (char *fileName)
{
	...
	while (fgets (temp, 512, pfile) != NULL)
	{
		...
		for (i = 0; i < upgrade_file_status[upgrade_num_files].numDevices;
			 i++)
        {
            line_tok = (char *)strtok (NULL, " ");
            ...
            memcpy (&upgrade_file_status[upgrade_num_files].deviceList[i],
                    &extAddr, 8);
        }
...
static void register_single_file (int i)
{
	...
	if (pkt)
	{
		...
		pkt->header.subsystem = ZSTACK_OTASYS_IDS__RPC_SYS_PB_OTA_MGR;
		pkt->header.cmd_id =
			OTA_MGR_CMD_ID_T__OTA_UPDATE_IMAGE_REGISTERATION_REQ;

		ota_update_image_registeration_req__pack (&pMsg,
													pkt->
													packed_protobuf_packet);
		UI_PRINT_LOG ("Sending packet len %x subsystem %d cmdid %d", len,
						pkt->header.subsystem, pkt->header.cmd_id);

		if (si_send_packet
			(pkt, (confirmation_processing_cb_t) & upgrade_process_regis_cnf,
			 (void *)i) != 0)
        {
            UI_PRINT_LOG
                ("register_single_file: Error: Could not send msg");
        }
