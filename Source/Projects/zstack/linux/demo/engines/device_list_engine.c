 ...
 * Function Prototypes
 *****************************************************************************/
static inline bool update_device_table_entry (device_info_t * entry,
											NwkDeviceInfoT * dev_info);
static inline bool update_endpoint_entry (endpoint_info_t * ep_info,
...
void device_process_list_response (pkt_buf_t * pkt, void *cbarg)
{
	...
	UI_PRINT_LOG
		("device_process_list_response: Received NWK_GET_DEVICE_LIST_CNF");

	msg =
		nwk_get_device_list_cnf__unpack (NULL, pkt->header.len,
	...
	if (msg)
	{
		if (msg->status == NWK_STATUS_T__STATUS_SUCCESS)
        {
            ...
            {
                UI_PRINT_LOG
                    ("device_process_list_response: Error: Number of devices exceed MAX");
                ds_devices_total = MAX_DEVICE_LIST;
            }

            UI_PRINT_LOG ("device_process_list_response: Total Devices %d",
                        ds_devices_total);
            ...
            {
                if (false ==
                    update_device_table_entry (&ds_device_table[i],
                                                    msg->devicelist[i - 1]))
                {
                    device_invalidate_entry (&ds_device_table[i]);
                    UI_PRINT_LOG
                        ("device_process_list_response: Error populating Device Table for device 0x%Lx",
                            msg->devicelist[i - 1]->ieeeaddress);
                }
            }
        ...
        {
            UI_PRINT_LOG
                ("device_process_list_response: Error: Status FAILURE.");
        }
	...
	{
		UI_PRINT_LOG
			("device_process_list_response: Error Could not unpack msg");
	}
}
...
void device_process_local_info_response (pkt_buf_t * pkt, void *cbarg)
{
	...
	UI_PRINT_LOG
		("device_process_local_info_response: Received NWK_GET_LOCAL_DEVICE_INFO_CNF");

	msg =
		nwk_get_local_device_info_cnf__unpack (NULL, pkt->header.len,
											 pkt->packed_protobuf_packet);
	...
	if (msg)
	{
		...
		if (false ==
			update_device_table_entry (&ds_device_table[0],
										 msg->deviceinfolist))
        {
            device_invalidate_entry (&ds_device_table[0]);
            UI_PRINT_LOG
                ("device_process_local_info_response: Error populating Device Table for device 0x%Lx",
                    msg->deviceinfolist->ieeeaddress);
        }
	...
	{
		UI_PRINT_LOG
			("device_process_local_info_response: Error Could not unpack msg");
	}
}
...
void device_process_change_indication (pkt_buf_t * pkt)
{
	...
	UI_PRINT_LOG
		("device_process_change_indication: Received NWK_ZIGBEE_DEVICE_IND");

	msg =
		nwk_zigbee_device_ind__unpack (NULL, pkt->header.len,
									 pkt->packed_protobuf_packet);
	...
	if (msg)
	{
		...
		for (i = 0; i < MAX_DEVICE_LIST; i++)
        {
            ...
            if ((free_index == -1) && (!ds_device_table[i].valid))
            {
                free_index = i;
                        ...
                        && (ds_device_table[i].device_status ==
                            NWK_DEVICE_STATUS_T__DEVICE_REMOVED))
            {
                removed_index = i;
            }
            ...
            if (ds_device_table[i].ieee_addr ==
                msg->deviceinfo->ieeeaddress)
            {
                found = true;
                break;
            }
		...
		if (found)
        {
            ...
            UI_PRINT_LOG
                ("device_process_change_indication: Found existing entry at index %d",
                    i);

            if (msg->deviceinfo->devicestatus ==
                NWK_DEVICE_STATUS_T__DEVICE_REMOVED)
            {
                device_invalidate_entry (&ds_device_table[i]);
                UI_PRINT_LOG
                    ("device_process_list_response: Device removed");
            }
            else
            {
                if (false ==
                                                    ...
                                                    msg->deviceinfo))
                {
                    device_invalidate_entry (&ds_device_table[i]);
                    UI_PRINT_LOG
                        ("device_process_list_response: Error populating Device Table, skipping device 0x%Lx",
                            msg->deviceinfo->ieeeaddress);
                }
                else
                {
		...
		else
        {
            if (free_index != -1)
            {
                ...
                UI_PRINT_LOG
                    ("device_process_change_indication: Adding new entry at index %d",
                        free_index);

                //Use free_index location for new entry and increment ds_devices_total
                if (false ==
                    update_device_table_entry (&ds_device_table
                                                    [free_index],
                                                    msg->deviceinfo))
                    {
                        device_invalidate_entry (&ds_device_table
                                                    [free_index]);
                        UI_PRINT_LOG
                            ("device_process_change_indication: Error populating Device Table, skipping device 0x%Lx",
                                msg->deviceinfo->ieeeaddress);
                    }
                else
                    {
            ...
            else
            {
                ...
                UI_PRINT_LOG
                    ("device_process_change_indication: ERROR Device Table Full");
            }
        }
...
void device_send_list_request (void)
{
	...
	if (pkt)
	{
		...
		if (si_send_packet
			(pkt,
			 (confirmation_processing_cb_t) & device_process_list_response,
			 NULL) != 0)
        {
            UI_PRINT_LOG
                ("device_send_list_request: Error: Could not send msg");
        }
...
void device_send_local_info_request (void)
{
	...
	UI_PRINT_LOG
		("device_send_local_device_info_request: Sending NWK_GET_LOCAL_DEVICE_INFO_REQ");
	...
	if (pkt)
	{
		...
		nwk_get_local_device_info_req__pack (&msg,
											 pkt->packed_protobuf_packet);

		if (si_send_packet
			(pkt,
			 (confirmation_processing_cb_t) &
			 device_process_local_info_response, NULL) != 0)
        {
            UI_PRINT_LOG
                ("device_send_local_device_info_request: Error: Could not send msg");
        }
	...
	{
		UI_PRINT_LOG
			("device_send_local_device_info_request: Error: Could not pack msg");
	}
}
											...
											NwkDeviceInfoT * dev_info)
{
	...
	{
		UI_PRINT_LOG
			("update_device_table_entry: Error: Number of endpoints exceed MAX value configured (types.h: MAX_ENDPOINTS).");

		return false;
	}

	UI_PRINT_LOG
		("update_device_table_entry: Adding/Updating enry info nwk_addr 0x%x ieeeaddr 0x%Lx manu id 0x%x status %d num endpoints %d",
	...
	{
		if (false ==
			update_endpoint_entry (&(entry->ep_list[j]),
									 dev_info->simpledesclist[j]))
        {
            return false;
        }
	}
...
static inline bool update_endpoint_entry (endpoint_info_t * ep_info,
										NwkSimpleDescriptorT * pbuf_desc)
{
	int k = 0;
	int l = 0;
	...
	if ((ep_info->num_ip_clusters > MAX_CLUSTERS_ON_EP)
		|| (ep_info->num_op_clusters > MAX_CLUSTERS_ON_EP))
	{
		UI_PRINT_LOG
			("update_endpoint_entry: Error: Number of clusters exceed MAX value configured (types.h: MAX_CLUSTERS_ON_EP).");
