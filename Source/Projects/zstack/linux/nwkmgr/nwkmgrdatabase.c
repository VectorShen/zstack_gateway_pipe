...
/*********************
  Local Prototypes
**********************/
static int nwkMgrDb_ComposeDeviceInfoRecord (char *pRec,
											 sNwkMgrDb_DeviceInfo_t *
											 pDeviceInfo);
static int nwmMgrDb_ComposeEndpointRecord (char *pRec, uint64_t ieeeAddr,
...
static int nwkMgrDb_FillDevice (char *rec,
...
bool nwkMgrDb_Init (bool consolidate)
{
	...
	gpNwkMgrDb_DeviceInfo = sdb_init_db ((char *)gszDbDeviceInfoPath,
										 ...
										 sdbtErrorComment, SDB_TYPE_TEXT,
										 strlen (gszNwkMgrDb_DeviceInfoHeader),
										 (char *)gszNwkMgrDb_DeviceInfoHeader);

	if (gpNwkMgrDb_DeviceInfo)
	{
		uiPrintf ("Opened Database '%s', %d records\n", gszDbDeviceInfoPath,
					sdbtGetRecordCount (gpNwkMgrDb_DeviceInfo));
	}
	else
	{
	...
	gpNwkMgrDb_Endpoints = sdb_init_db ((char *)gszDbEndpointsPath,
										...
										(consolidation_processing_f)
										sdbtErrorComment, SDB_TYPE_TEXT,
										strlen (gszNwkMgrDb_EndpointsHeader),
										(char *)gszNwkMgrDb_EndpointsHeader);

	if (gpNwkMgrDb_Endpoints)
	{
		uiPrintf ("Opened Database '%s', %d records\n", gszDbEndpointsPath,
					sdbtGetRecordCount (gpNwkMgrDb_Endpoints));
	}
	else
	{
		uiPrintf ("Failed to open Database '%s'\n", gszDbEndpointsPath);
...
bool nwkMgrDb_AddDevice (sNwkMgrDb_DeviceInfo_t * pDeviceInfo)
{
	...
	sdb_delete_records (gpNwkMgrDb_DeviceInfo, &pDeviceInfo->ieeeAddr,
						nwkMgrDb_MatchDeviceIeeeAddr);
	sdb_delete_records (gpNwkMgrDb_Endpoints, &pDeviceInfo->ieeeAddr,
						nwkMgrDb_MatchDeviceIeeeAddr);
	...
	{
		// add entry
		nwmMgrDb_ComposeEndpointRecord (szRecord, pDeviceInfo->ieeeAddr,
										&(pDeviceInfo->aEndpoint[i]));
...
bool nwkMgrDb_RemoveDevice (uint64_t ieeeAddr)
{
	...
	// remove the device
	rec =
		sdb_delete_record (gpNwkMgrDb_DeviceInfo, &ieeeAddr,
						 nwkMgrDb_MatchDeviceIeeeAddr);
	if (!rec)
	{
		return FALSE;
	...
	do
	{
		...
		rec =
			sdb_delete_record (gpNwkMgrDb_Endpoints, &ieeeAddr,
								 nwkMgrDb_MatchDeviceIeeeAddr);
...
static int nwkMgrDb_FillDevice (char *rec, sNwkMgrDb_DeviceInfo_t * pDeviceInfo)
{
	...
	sdb_txt_parser_get_uint64_field (&rec, &(pDeviceInfo->ieeeAddr),
									 &parsingResult);
	sdb_txt_parser_get_numeric_field (&rec, &(pDeviceInfo->nwkAddr), 2, FALSE,
									&parsingResult);
	sdb_txt_parser_get_numeric_field (&rec, &(pDeviceInfo->status), 1, FALSE,
									&parsingResult);
	sdb_txt_parser_get_numeric_field (&rec, &(pDeviceInfo->manufacturerId), 2,
									FALSE, &parsingResult);
	sdb_txt_parser_get_numeric_field (&rec, &(pDeviceInfo->endpointCount), 1,
									FALSE, &parsingResult);
	sdb_txt_parser_get_uint64_field (&rec, &(pDeviceInfo->parentAddr),
									 &parsingResult);

	// allocate endpoints, if we can't we can't continue parsing
	pDeviceInfo->aEndpoint =
		malloc (pDeviceInfo->endpointCount * sizeof (sNwkMgrDb_Endpoint_t));
	...
	for (ep = 0; ep < pDeviceInfo->endpointCount; ++ep)
	{
		...
		epRec =
			sdb_get_record (gpNwkMgrDb_Endpoints, &ieeeAddr,
							nwkMgrDb_MatchDeviceIeeeAddr, &context);
		if (!epRec)
        {
            break;
        }
		...
		sdb_txt_parser_get_numeric_field (&pBuf, &(pEndpoint->endpointId), 1,
											FALSE, &parsingResult);
		sdb_txt_parser_get_numeric_field (&pBuf, &(pEndpoint->profileId), 2,
											FALSE, &parsingResult);
		sdb_txt_parser_get_numeric_field (&pBuf, &(pEndpoint->deviceId), 2,
											FALSE, &parsingResult);
		sdb_txt_parser_get_numeric_field (&pBuf, &(pEndpoint->deviceVer), 1,
											FALSE, &parsingResult);
		sdb_txt_parser_get_numeric_field (&pBuf,
											&(pEndpoint->inputClusterCount), 1,
											FALSE, &parsingResult);
		sdb_txt_parser_get_numeric_field (&pBuf,
											&(pEndpoint->outputClusterCount), 1,
											FALSE, &parsingResult);

		// allocate memory for input clusters. If it doesn't work, then we can't process this endpoint
		pEndpoint->inputClusters =
			malloc (pEndpoint->inputClusterCount * sizeof (uint16));
		...
		for (i = 0; i < pEndpoint->inputClusterCount && !parsingResult.code;
			 ++i)
        {
            sdb_txt_parser_get_numeric_field (&pBuf, &cluster, 2, FALSE,
                                            &parsingResult);
		...
		// get output clusters
		for (i = 0; i < pEndpoint->outputClusterCount && !parsingResult.code;
			 ++i)
        {
            sdb_txt_parser_get_numeric_field (&pBuf, &cluster, 2, FALSE,
                                            &parsingResult);
...
sNwkMgrDb_DeviceInfo_t *nwkMgrDb_GetDeviceInfo (uint64_t ieeeAddr)
{
	...
	rec =
		sdb_get_record (gpNwkMgrDb_DeviceInfo, &ieeeAddr,
						nwkMgrDb_MatchDeviceIeeeAddr, NULL);
	if (!rec)
	{
 ...
 *
 **************************************************************************************************/
static void nwkMgrDb_FreeDeviceInfoSubstructures (sNwkMgrDb_DeviceInfo_t *
												pDeviceInfo)
{
	int ep;
...
sNwkMgrDb_DeviceList_t *nwkMgrDb_GetAllDevices (void)
{
	...
	pDeviceList->pDevices =
		malloc (pDeviceList->count * sizeof (sNwkMgrDb_DeviceInfo_t));
...
void nwkMgrDb_FreeAllDevices (sNwkMgrDb_DeviceList_t * pDeviceList)
{
	...
	if (pDeviceList)
	{
		if (pDeviceList->pDevices)
        {
            ...
            {
                nwkMgrDb_FreeDeviceInfoSubstructures (&pDeviceList->
                                                        pDevices[i]);
            }
...
bool nwkMgrDb_GetShortAddr (uint64_t ieeeAddr, uint16 * pShortAddr)
{
	...
	if (rec)
	{
		...
		sdb_txt_parser_get_numeric_field (&pBuf, pShortAddr, 2, FALSE,
											&parsingResult);
		sdb_release_record ((void **)(&rec));
...
bool nwkMgrDb_GetParentAddr (uint64_t ieeeAddr, uint64_t * pParentIeeeAddr)
{
	...
	if (rec)
	{
		...
		sdb_txt_parser_get_uint64_field (&pBuf, pParentIeeeAddr,
										 &parsingResult);
...
bool nwkMgrDb_SetCapInfo (uint64_t ieeeAddr, uint8 capInfo)
{
	...
	sdb_delete_records (gpNwkMgrDb_DeviceInfo, &ieeeAddr,
						nwkMgrDb_MatchDeviceIeeeAddr);
...
bool nwkMgrDb_GetIeeeAddr (uint16 shortAddr, uint64_t * pIeeeAddr)
{
	...
	// find the record
	rec =
		sdb_get_record (gpNwkMgrDb_DeviceInfo, &shortAddr,
						nwkMgrDb_MatchDeviceShortAddr, NULL);
	if (rec)
	{
		pBuf = rec;
...
static int nwkMgrDb_MatchDeviceShortAddr (void *record, void *key)
{
	...
	sdb_txt_parser_get_numeric_field (&rec, &shortAddr, 2, FALSE,
									&parsingResult);

	if (parsingResult.code == SDB_TXT_PARSER_RESULT_OK
		&& (shortAddr == *pShortAddrToMatch))
	{
											 ...
											 pDeviceInfo)
{
	...
	// IeeeAddress,NwkAddress,Status,MfgId,EP_Count,ParentIeeeAddress
	sprintf (pRec,
			 "        %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X , 0x%04X , 0x%02X, 0x%04X , %d, %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X, 0x%02X\n",
			 ...
			 BREAK_UINT64 (pDeviceInfo->ieeeAddr, 0), pDeviceInfo->nwkAddr,
			 pDeviceInfo->status, pDeviceInfo->manufacturerId,
			 pDeviceInfo->endpointCount, BREAK_UINT64 (pDeviceInfo->parentAddr,
													 7),
			 ...
			 BREAK_UINT64 (pDeviceInfo->parentAddr, 0), pDeviceInfo->capInfo);
										 ...
										 sNwkMgrDb_Endpoint_t * pEndpoint)
{
	...
	pRec +=
		sprintf (pRec,
				 "        %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X , %d , 0x%04X, 0x%04X , %d , %d , %d",
				 BREAK_UINT64 (ieeeAddr, 7), BREAK_UINT64 (ieeeAddr, 6),
				 BREAK_UINT64 (ieeeAddr, 5), BREAK_UINT64 (ieeeAddr, 4),
				 BREAK_UINT64 (ieeeAddr, 3), BREAK_UINT64 (ieeeAddr, 2),
				 BREAK_UINT64 (ieeeAddr, 1), BREAK_UINT64 (ieeeAddr, 0),
				 pEndpoint->endpointId, pEndpoint->profileId,
				 pEndpoint->deviceId, pEndpoint->deviceVer,
...
void UtPrintDevice (sNwkMgrDb_DeviceInfo_t * pDeviceInfo)
{
	...
	uiPrintf ("Found Device %016llX, %d endpoints: ", pDeviceInfo->ieeeAddr,
...
{
	static const sNwkMgrDb_NetworkInfo_t sNwkInfo =
		{ 1, 26, 0x1A3E, 0x1122334455667788ULL };
	sNwkMgrDb_DeviceList_t *pDeviceList;
	sNwkMgrDb_DeviceInfo_t *pDeviceInfo;
	int i;
	uint16 shortAddr;
