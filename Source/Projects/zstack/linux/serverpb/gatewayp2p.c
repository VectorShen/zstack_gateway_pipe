...
static SrvrSimpleDescriptorT **gwPb_HandleServerSimpleDescPb (int
															simpleDescCount,
...
bool gwPb_SrvrGetIeeeAddress (uint16 shortAddr, uint64_t * pIeeeAddr)
{
	...
	pRsp =
		apicSendSynchData (GW_NWKMGR_HANDLE,
						 Z_STACK_SERVER_SYS_ID_T__RPC_SYS_PB_SRVR,
						 SRVR_CMD_ID_T__SRVR_GET_IEEE_ADDRESS_REQ, len, pBuf,
	...
	{
		if ((rspLen > 0)
			&& (rspCmdId == SRVR_CMD_ID_T__SRVR_GET_IEEE_ADDRESS_CNF))
        {
            pIeeeAddrCnf =
                srvr_get_ieee_address_cnf__unpack (NULL, rspLen, pRsp);
            if (pIeeeAddrCnf)
            {
                ...
                srvr_get_ieee_address_cnf__free_unpacked (pIeeeAddrCnf,
                                                            NULL);
            }
        }
		else
...
bool gwPb_SrvrGetShortAddress (uint64_t ieeeAddr, uint16 * pShortAddr)
{
	...
	pRsp =
		apicSendSynchData (GW_NWKMGR_HANDLE,
						 Z_STACK_SERVER_SYS_ID_T__RPC_SYS_PB_SRVR,
						 SRVR_CMD_ID_T__SRVR_GET_SHORT_ADDRESS_REQ, len, pBuf,
						 NULL, &rspCmdId, &rspLen);
	...
	{
		if ((rspLen > 0)
			&& (rspCmdId == SRVR_CMD_ID_T__SRVR_GET_SHORT_ADDRESS_CNF))
        {
            pShortAddrCnf =
                srvr_get_short_address_cnf__unpack (NULL, rspLen, pRsp);
            if (pShortAddrCnf)
            {
                if (pShortAddrCnf->status !=
                    SRVR_STATUS_T__STATUS_SUCCESS)
                {
                    uiPrintf
                ...
                srvr_get_short_address_cnf__free_unpacked (pShortAddrCnf,
                                                                NULL);
            }
        }
		else
...
SrvrDeviceInfoT *gwPb_SrvrGetDeviceInfoReq (uint64_t ieeeAddr)
{
	...
	pRsp =
		apicSendSynchData (GW_NWKMGR_HANDLE,
						 Z_STACK_SERVER_SYS_ID_T__RPC_SYS_PB_SRVR,
						 SRVR_CMD_ID_T__SRVR_GET_DEVICE_INFO_REQ, len, pBuf,
						 NULL, &rspCmdId, &rspLen);
	...
	{
		if ((rspLen > 0)
			&& (rspCmdId == SRVR_CMD_ID_T__SRVR_GET_DEVICE_INFO_CNF))
        {
            pDeviceInfoCnf =
                srvr_get_device_info_cnf__unpack (NULL, rspLen, pRsp);
            if (pDeviceInfoCnf)
            {
                // Check the return status
                if (pDeviceInfoCnf->status !=
                    SRVR_STATUS_T__STATUS_SUCCESS)
                {
                    uiPrintf
                ...
                else
                {
                    ...
                    pDeviceInfo->networkaddress =
                        pDeviceInfoCnf->deviceinfo->networkaddress;
                    pDeviceInfo->ieeeaddress =
                        pDeviceInfoCnf->deviceinfo->ieeeaddress;
                    pDeviceInfo->has_parentieeeaddress =
                        pDeviceInfoCnf->deviceinfo->
                        has_parentieeeaddress;
                    pDeviceInfo->parentieeeaddress =
                        pDeviceInfoCnf->deviceinfo->parentieeeaddress;
                    pDeviceInfo->manufacturerid =
                        pDeviceInfoCnf->deviceinfo->manufacturerid;
                    pDeviceInfo->n_simpledesclist =
                        pDeviceInfoCnf->deviceinfo->n_simpledesclist;
                    pDeviceInfo->devicestatus =
                        pDeviceInfoCnf->deviceinfo->devicestatus;

                    if (pDeviceInfo->n_simpledesclist)
                    {
                        // Fill in simple descriptor
                        pDeviceInfo->simpledesclist =
                            gwPb_HandleServerSimpleDescPb
                            (pDeviceInfoCnf->deviceinfo->
                                n_simpledesclist,
                                pDeviceInfoCnf->deviceinfo->
                                simpledesclist);
...
bool gwPb_SrvrGetDeviceStatus (uint64_t ieeeAddr, uint8 * pStatus)
{
	...
	pRsp =
		apicSendSynchData (GW_NWKMGR_HANDLE,
						 Z_STACK_SERVER_SYS_ID_T__RPC_SYS_PB_SRVR,
						 SRVR_CMD_ID_T__SRVR_GET_DEVICE_STATUS_REQ, len, pBuf,
						 NULL, &rspCmdId, &rspLen);
	...
	{
		if ((rspLen > 0)
			&& (rspCmdId == SRVR_CMD_ID_T__SRVR_GET_DEVICE_STATUS_CNF))
        {
            pDeviceStatusCnf =
                srvr_get_device_status_cnf__unpack (NULL, rspLen, pRsp);
            if (pDeviceStatusCnf)
            {
                if (pDeviceStatusCnf->status !=
                    SRVR_STATUS_T__STATUS_SUCCESS)
                {
                    uiPrintf
                ...
                srvr_get_device_status_cnf__free_unpacked
                    (pDeviceStatusCnf, NULL);
            }
        }
		else
        {
...
bool gwPb_SrvrSetDeviceStatus (uint64_t ieeeAddr, uint8 status)
{
	...
	pRsp =
		apicSendSynchData (GW_NWKMGR_HANDLE,
						 Z_STACK_SERVER_SYS_ID_T__RPC_SYS_PB_SRVR,
						 SRVR_CMD_ID_T__SRVR_SET_DEVICE_STATUS_REQ, len, pBuf,
						 NULL, &rspCmdId, &rspLen);
	...
	{
		if ((rspLen > 0)
			&& (rspCmdId == SRVR_CMD_ID_T__SRVR_SET_DEVICE_STATUS_CNF))
        {
            pDeviceStatusCnf =
                srvr_set_device_status_cnf__unpack (NULL, rspLen, pRsp);
            if (pDeviceStatusCnf)
            {
                if (pDeviceStatusCnf->status !=
                    SRVR_STATUS_T__STATUS_SUCCESS)
                ...
                srvr_set_device_status_cnf__free_unpacked
                    (pDeviceStatusCnf, NULL);
            }
        }
		else
