...
db_descriptor *sdb_init_db (char *name, get_record_size_f get_record_size,
							...
							mark_deleted_f mark_deleted,
							consolidation_processing_f consolidation_processing,
							uint8 db_type, uint32 db_header_size, void *header)
{
...
bool sdb_modify_last_accessed_record (db_descriptor * _db, void *record)
{
	...
	worked = !((db->get_record_size (record) != db->last_accessed_record_size)
			 ||
			 ((fseek
				 (db->file, db->last_accessed_record_start_file_pointer,
				SEEK_SET) != 0)
				|| (fwrite (record, db->last_accessed_record_size, 1, db->file)
					!= 1) || (fflush (db->file) != 0)));
	sdb_unlock (db);
...
bool sdb_consolidate_db (db_descriptor ** _db)
{
	...
	int context;
	int rc;
	static char tempfilename[MAX_SUPPORTED_FILENAME +
							 TEMP_FILENAME_EXTENTION_LENGTH + 1];
	...
	{
		uiPrintf
			("%s: Warning: Failed to delete old temp db (errno=%d), %s\n",
			 __func__, errno, tempfilename);
		return TRUE;
	}

	tempDb =
		sdb_init_db (tempfilename, db->get_record_size, db->check_deleted,
					 db->check_ignore, db->mark_deleted,
					 db->consolidation_processing, db->type, db->db_header_size,
					 db->header);
								 ...
								 parsingResult_t * result)
{
	...
	{
		for (i = 0; (result->code == SDB_TXT_PARSER_RESULT_OK) && (i < len);
			 i++)
        {
...
void sdb_txt_parser_get_numeric_field (char **pBuf, void *field, uint8 len,
									 bool isSigned, parsingResult_t * result)
{
	union
	{
		signed long sNum;
	...
	else if (result->code == SDB_TXT_PARSER_RESULT_OK)
	{
		...
		while (base)
        {
            if (isSigned)
            {
                ...
                if ((errno == ERANGE)
                    || (temp.sNum >
                        (((signed long)0x7FFFFFFF) << (8 * (4 - len))))
                    || (temp.sNum <
                        (((signed long)0x80000000) << (8 * (4 - len)))))
                {
                    result->code =
                        SDB_TXT_PARSER_RESULT_VALUE_OUT_OF_RANGE;
                    result->errorLocation = (*pBuf) - 1;
            ...
            else
            {
                ...
                if ((errno == ERANGE)
                    || (temp.uNum >
                        (((unsigned long)0xFFFFFFFF) >> (8 * (4 - len)))))
                {
                    result->code =
                        SDB_TXT_PARSER_RESULT_VALUE_OUT_OF_RANGE;
                    result->errorLocation = (*pBuf) - 1;
