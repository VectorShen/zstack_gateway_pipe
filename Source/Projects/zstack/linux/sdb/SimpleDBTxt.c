...
{
	return ((((char *)record)[0] == SDBT_COMMENT_CHARACTER)
		...
		if (rc)
        {
            ...
            if (parsingResult.code == SDB_TXT_PARSER_RESULT_OK)
            {
                sprintf (comment, "%cERROR:%*s %s\n",
                            SDBT_COMMENT_CHARACTER, errorOffset - 7 + 1, "^",
                            parsingErrorStrings[errorCode]);
            }
            else
            {
                sprintf (comment, "%c%*s %s (%s)\n",
                            SDBT_COMMENT_CHARACTER,
                            (int)(parsingResult.errorLocation - record) - 1 +
                            1, "^", "BAD_ERROR_DESCRIPTION_HEADER",
